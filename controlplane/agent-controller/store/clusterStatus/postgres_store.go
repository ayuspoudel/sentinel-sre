...
func (s *StatusStore) Upsert(ctx context.Context, st *clusterStatusModel.ClusterStatus) error {
...
func (s *StatusStore) Get(ctx context.Context, clusterName string) (*clusterStatusModel.ClusterStatus, error) {
	query := `SELECT
		cluster_name,
		agent_id,
		last_reconcile_at,
	...
	var st clusterStatusModel.ClusterStatus
	err := row.Scan(
		&st.ClusterName,
		&st.AgentID,
		&st.LastReconcileAt,
