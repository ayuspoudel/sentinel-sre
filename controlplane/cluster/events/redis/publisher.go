...
import (
	...
	"github.com/ayuspoudel/sentinel-sre/controlplane/cluster/events/clusterDeleted"
...
var _ events.ClusterIntentPublisher = (*RedisPublisher)(nil)
...
func (p *RedisPublisher) PublishClusterRegistered(ctx context.Context, c *model.Cluster) error {
	...
	clusterData := clusterRegistered.FromCluster(c)
	event := clusterRegistered.NewClusterDataEvent(clusterData)
...
func (p *RedisPublisher) PublishClusterDeleted(ctx context.Context, clusterName string) error {
	if clusterName == "" {
		return fmt.Errorf("empty cluster name")
	}
	event := clusterDeleted.NewClusterDeletedEvent(clusterName)
	payload, err := json.Marshal(event)
	if err != nil {
		return fmt.Errorf("failed to marshal cluster deleted event: %w", err)
	}
	args := &redis.XAddArgs{
		Stream: p.streamName,
		ID:     "*",
	...
	err = p.client.XAdd(ctx, args).Err()
	if err != nil {
		return fmt.Errorf("failed to publish cluster deleted event to redis: %w", err)
	}
	return nil
